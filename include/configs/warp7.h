...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"boot_script_signed=boot.scr.imx-signed\0" \
	"image=zImage\0" \
	"console=ttymxc0\0" \
	"ethact=usb_ether\0" \
	...
	"do_bootscript_hab=" \
		"if test ${hab_enabled} -eq 1; then " \
			...
			"setenv boot_scripts ${boot_script_signed}; " \
			"load mmc ${mmcdev}:${mmcpart} ${hab_ivt_addr} ${boot_scripts}; " \
